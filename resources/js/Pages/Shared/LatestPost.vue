...
export default {
    components: {Pagination, Link, Head, Layout, moment},
    data() {
        return {
            indexInArray: 0,
            perPage: 6,
    ...
    watch:{
      search(x){
          ...
          this.$inertia.get('/latest',{search: x},{
              preserveState: true,
              preserveScroll: true,
              replace: true
          })
...
<template>
    <Layout>
        ...
        <div class="flex flex-row justify-end w-full">
            <p class="w-auto mr-2">Post's counter: {{counterPosts}}</p>
        </div>
        ...
        <div class="flex justify-center">
            <form class="w-96">
                ...
                <div >
                    <div class="relative top-9 inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
        ...
        <div class="flex flex-col items-center">
            <br>
            <p>The biggest trophy</p>
            <p>Post with higher ratings</p>
            <p>LATEST ADDED POST ON SITE:</p>
            <br>
            <div class="grid grid-cols-3  content-center gap-2 w-3/5">
                <div v-for="post in test.data" class="border-l-blue-700 text-red-800 m-2 mb-8">
                    <Link

                        :href="`/readpost/${post.id}`" class="link">
                        <p class="text-nowrap truncate ...">Title post: {{ post.title_post }}</p>

                        <div v-if="post.tilePath">
                            <img :src="`/storage/${post.tilePath.path}`"
                                 class="rounded h-2/4 w-full h-3/4"
                            >
                        </div>
                        <div v-else>
                            <img
